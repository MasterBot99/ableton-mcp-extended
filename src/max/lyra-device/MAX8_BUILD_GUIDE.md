# Lyra — Max for Live Device: Save & Deploy
...
## Schritt 5 — Als .amxd speichern

**Wichtig:** Die `.amxd` muss im gleichen Ordner wie `bridge-client.js` und `node_modules/` liegen, sonst findet `node.script` das Script nicht.

1. Im Patch: **File → Save As**
2. Format: **Max for Live Device** (`.amxd`)
3. Dateiname: `Lyra.amxd`
...
2. Browser → **Max for Live** → `Lyra` sollte erscheinen
   - Falls nicht: In Max 8 das Device öffnen und nochmal als `.amxd` speichern
3. `Lyra` auf einen MIDI-Track ziehen
4. Device-View:
...
| `.amxd` lädt nicht in Live | Device in Max 8 öffnen und neu speichern, Browser-Cache leeren |
...
1. `Lyra.amxd` + `bridge-client.js` + `node_modules/` in einem Ordner bündeln
...
Die `.amxd` selbst ist proprietär und wird in Max gespeichert.
